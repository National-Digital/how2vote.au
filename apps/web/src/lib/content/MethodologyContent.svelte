<script lang="ts">
  ...
  // The body of the /methodology page. It lives here so the SAME markup renders on the page and in
  // the dialog raised by an in-flow `<DocLink href="/methodology">` — a second copy would drift,
  // and this is the document the app cites whenever it says "see how the scoring works".
  //
  // The page's JSON-LD (StructuredData / methodologyHowToGraph) deliberately stays on the ROUTE:
  // it describes the document at /methodology, and emitting it again from a dialog on some other
  // page would claim that page is the HowTo.
  import { HOWTO_STEPS } from "$lib/structured-data";
...
<p>
  ...
  and every step below is exactly what the scoring engine computes. Which issues to include and how to
  describe a division still involve editorial judgement, so this is one reasonable reading of the public
  record, not the only one.
</p>
...
<p>
  Every <GlossaryTerm id="division">division</GlossaryTerm> (formal vote) in the House and Senate is recorded
  in <GlossaryTerm id="hansard">Hansard</GlossaryTerm> and published, in a machine-readable form, by the
  OpenAustralia Foundation's
  <ExternalLink href="https://theyvoteforyou.org.au/">They Vote For You</ExternalLink>. For each
  issue, They Vote For You gives an <strong>agreement figure from 0 to 100</strong> for every member —
  how often that member voted the way the issue describes. We use those figures under the Open Database
  Licence (ODbL), and we credit They Vote For You on every screen and every printed card.
</p>
...
<p>
  ...
  of that party's members (each member counted once), then place the party on a five-point scale by dividing
  0–100 into five equal bands:
</p>
<ul>
...
<h2>Your answers</h2>
<p>
  You answer each proposition on the same five-point scale (1 strongly disagree … 5 strongly agree),
  or skip it. When you review your answers at the end, you can <strong>star</strong> the issues that
  matter most to you as <strong>extremely important</strong>, which multiplies that one question's
  weight by <strong>exactly ten</strong>. Only your two strongest answers —
...
<p>
  Marking a strong answer <strong>extremely important</strong> multiplies both the points earned and
  the points at stake by ten — up to <code>40</code> for that question:
...
<p>
  Across every question you answered, we add up the points each party earned and the points it could
  have earned. Parties that have merged or been renamed between parliaments are combined
  <strong>at this stage</strong> — we add their raw earned and possible points together first, then compute
  a single percentage, so a merged party is treated as one continuous entity rather than an average of
  two rounded scores. The match is then
</p>
...
<p>
  ...
  nothing is ranked, nothing is crowned, and How2Vote never suggests who to put first. Every party's
  score links to the divisions behind it, so you can check any number.
</p>
<p>
...
<p>
  ...
  six) or <strong>below the line</strong> (every individual candidate; number at least twelve), both in
  ballot order. Because our scoring is party-level, an above-the-line group takes the score of the party
  that heads it, and every candidate in a group shares that party's score. Number one way or the other
  — not both on the same paper.
</p>

<p>
  The scoring engine is open source and covered by tests that pin these numbers exactly; any change
  to the method is versioned and shown here and in the footer.
</p>

<h2>What the method does and doesn't capture</h2>
<p>
  The comparison rests on editorial and methodological choices, and it is one reasonable reading of
  the public record — not the only one:
</p>
<ul>
  <li>
  ...
  <li>
    ...
    yourself. The set of propositions and the reasons for choosing them are recorded as a versioned
  ...
  <li>
    <strong>Party-level records beside new candidates.</strong> A candidate with no personal voting record
    is shown against their party's historical record — that describes the party, not the candidate's own
    views, promises or future conduct.
  </li>
  <li>
    <strong>Ties.</strong> Equal alignment figures need no tie-break: candidates are never ranked by score,
    only ever shown in official ballot order.
  </li>
</ul>
...
<p>
  ...
  itself changes — the banding, the distance and points formula, the weighting, or how party records
  are combined — not when the data is simply refreshed.
</p>
<p>
  Each election's card records its <strong>data vintage</strong> (the latest division the positions
  were compiled from), the <strong>data</strong> and <strong>app</strong> versions, and the parliamentary
  voting source, so a saved or printed card carries its own provenance. The dataset also records the official
  source of its candidate ballots (AEC candidate nominations) and the data version they arrived in. Ballots
  are ingested through a manual, human-reviewed change process, and once an election's snapshot is locked,
  automated checks refuse any further change to it.
</p>
<p>
...
<p>
  People may also answer an optional survey about themselves. Its questions follow Australian Bureau
  of Statistics categories and Australian Election Study conventions, so the responses can be
  compared against Census and other public data; the exact wordings are recorded in our research
  codebook. We hold no name, contact detail, IP address or device identifier, and no per-person
  research record exists at all — though no technique eliminates every possible risk: while counts
  are small, a rare category could still say something about a person known to have contributed.
  That is why counts pairing results with sensitive answers are kept national-only, stored counts
  are kept apart from operational systems, and only privacy-protected aggregates are published — see

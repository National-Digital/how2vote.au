...
<ContentPage title="Privacy policy">
  ...
  <p>
    This information remains on that browser unless you choose an action that sends it, such as
    research contribution or a form submission. You can remove it using the Service's start-again
    and delete controls or by clearing site data in your browser.
  </p>

  <h2>4. Shared links and plans</h2>
  <p>
    A share link may encode your answers or results in the part of the address after the
    <code>#</code> symbol, known as the URL <strong>fragment</strong>. The Service is designed so
    that its servers and analytics do not receive or record that fragment; the recipient's browser
    uses it to reconstruct the shared result on their device.
  </p>
  <p>
  ...
  <p>
    ...
    After your result is available, you may be invited to contribute a research record.
    Participation is optional, is not required to use the Service and does not change your result or
    voting plan.
  </p>
  <p>
  ...
  <p>
    ...
    reduces each proposition you answered to agree, neutral or disagree. Your individual quiz
    answers and the weights you selected <strong>never leave your device</strong>. A contribution
    then contains only:
  </p>
  <ul>
    ...
    <li>
      the election being compared, together with that election's public AEC timetable dates, which
      the server uses to classify the period in which you contributed;
    </li>
    <li>
      the version of the consent notice you agreed to, which is kept only as an aggregate count of
      how many contributors accepted each version, never against your other answers;
    </li>
    <li>
  ...
  <p>
    ...
    research record is created or stored, and counts that pair a result with a sensitive survey answer
    are kept at national level only, never by state.
  </p>
  <p>
  ...
  <p>
    Your IP address and other technical data may nevertheless be processed briefly by hosting,
    network and security providers to transmit and protect the request. We configure the research
    system so that those technical details are not copied into the research dataset.
  </p>
  ...
  <p>
    ...
    near any result, electorate is kept only as a <strong>running count</strong> — a tally of how many
    contributions come from each electorate, with no results, survey answers, dates or anything else attached
    — sent by your browser as its own separate request that is not joined to your contribution in our
    storage. No count ever pairs an electorate with a result or a survey answer. (Because two requests
    from the same device travel the same network path, we describe this as separated by design rather
    than as an absolute guarantee of unlinkability.)
  </p>
  ...
  <p>
    ...
    contact participants, determine eligibility for a service, or make decisions about an
    individual.
  </p>
  ...
  <p>
    ...
    analysis plan. We do not collect a name, email address or account identifier with a
    contribution. However, no de-identification technique can eliminate every possible
    re-identification risk: while contributions are being gathered a group count can be small, and a
    very small count in a rare category could say something about a person known to have
    contributed, particularly if combined with information held elsewhere.
  </p>
  <p>
    We reduce that risk by keeping any count that pairs a result with a sensitive survey answer at
    national level only, restricting access to the counters themselves, separating research data
    from operational logs, limiting retention and publishing only aggregated results that meet
    disclosure thresholds. We do not attempt to re-identify participants and contractually prohibit
    service providers and authorised researchers from attempting to do so.
  </p>
  ...
  <p>
    ...
    >, so there is no individual record that could tie a stored count back to you. You control
    whether a contribution is ever made at all through the opt-in described above; nothing is sent
    unless you actively consent.
  </p>
  ...
  <p>
    ...
    individual record. Because those counts are genuinely aggregated statistics that do not relate
    to an identifiable individual, we may retain them <strong>indefinitely</strong> to support long-run
    repeated cross-sectional analysis, comparison across federal election datasets and collection periods,
    methodological validation and historical research. Participants are not identified or linked between
    elections, so this supports repeated cross-sectional and cross-election comparison rather than following
  ...
  <p>
    ...
    identifier, so it cannot be used to recognise or track you across visits or across sites.
    Because it collects no personal information and needs no cookie, there is nothing to switch on
    and no consent banner to dismiss.
  </p>
  ...
  <p>
    If you submit a contact or feedback form, we collect the information you enter, such as your
    name, email address and message. We use it to respond, investigate reports, maintain the Service
    and protect our legal rights. Form messages are normally retained for up to 24 months, unless
    they are needed for a continuing complaint, legal matter, security incident or recordkeeping
    obligation.
  </p>
  <p>
  ...
  <p>
    ...
    technical information such as your IP address, request time, browser, requested page and
    security signals. We use this information to deliver the website, prevent abuse, diagnose faults
    and investigate security incidents. We do not intentionally combine routine technical logs with
  ...
  <p>
    We use service providers to host, secure and operate the Service. They process information for
    us under their terms and contractual privacy and security commitments. This list is generated
    from our internal vendor register, so it reflects every service recorded in that register.
  ...
  <p>
    Before sending personal information to an overseas recipient, we take reasonable steps
    appropriate to the circumstances to ensure it is handled consistently with applicable Australian
    privacy requirements. We do not sell personal information or research records.
  </p>
  ...
  <p>
    ...
    information, including encrypted network connections, access controls, separation of research
    and operational systems, least-privilege access, dependency and vulnerability management,
    backups and incident-response procedures. No internet service or storage system can be
    guaranteed completely secure. If an eligible data breach occurs, we will assess and notify
    affected individuals and the Office of the Australian Information Commissioner where required by
    law.
  </p>

  <h2>11. Access, correction and deletion</h2>
  <p>
    You can view, change or delete locally stored answers and plans through the Service or your
    browser settings. The control below clears everything stored on this device in one step — your
    in-progress quiz, saved comparisons, selected election, theme, Terms acknowledgement, privacy
    choices and age-eligibility confirmation, together with the offline copy of the app — and it is
    also available on the <a href="/saved">Saved cards</a> page. It clears this device only: it cannot
    recall a link you have already shared, because a share link carries its answers inside the link itself
    and stays with whoever received it (see section 4).
  </p>
  <ClearMyData />
  <p>
    You may ask us to access or correct personal information we hold about you, or to delete it
    where applicable, by contacting <a href="mailto:{ORG.email}">{ORG.email}</a>. We may need to
    verify your identity before acting. As explained above, a de-identified research record holds
    nothing that could locate it, so we are unable to retrieve or delete an individual research
    record.
  </p>
  ...
  <p>
    ...
    research contributions from anyone under {RESEARCH_MIN_AGE}. A person must confirm that they are
    at least
  ...
  <p>
    ...
    version and effective date will be published on this page. Material changes to research
    collection or use will not be applied to a new research submission without a new or updated
